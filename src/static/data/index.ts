import profileicon from './profileicon.json';
import champion from './champion.json';
import item from './item.json';
import championMap from './championMap.json';
import summonerMap from './summonerMap.json';
import summoner from './summoner.json';
import queueMap from './queueData.json';
import rune from './runesReforged.json';
export * from './runesMainMap';
export * from './categories';

export { profileicon, champion, championMap, queueMap, item, summonerMap, summoner, rune };
